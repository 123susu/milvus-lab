...
test("wires the page to the local benchmark API with resilient states", async () => {
  ...
  assert.match(page, /analyzeRecommendations/);
  assert.match(page, /P99 推荐/);
  assert.match(page, /Recall 推荐/);
  assert.match(page, /内存推荐/);
  assert.match(page, /综合推荐/);
  assert.match(page, /当前索引配置归一化后等权得分最低/);
  assert.match(page, /INDEX PARAMETER RECOMMENDATIONS/);
  assert.match(page, /参数配置建议/);
  assert.match(page, /withinIndexRecommendations/);
  assert.match(page, /所有指标完整的配置均参与，不设置样本数门槛/);
  assert.doesNotMatch(page, /CROSS-INDEX RECOMMENDATIONS/);
  assert.doesNotMatch(page, /全部索引的推荐配置/);
  assert.doesNotMatch(page, /crossIndexRecommendations/);
  assert.doesNotMatch(page, /不同索引综合排名/);
  assert.doesNotMatch(page, /样本不足/);
  assert.doesNotMatch(page, /Pareto 最优/);
